...
int main() try {
    ...
    for (uint32_t i = 0; i < sensorList->getCount(); i++) {
        ...
        if (type == OB_SENSOR_COLOR) {
            ...
            // Pass 1 (preferred): exact MJPG @ COLOR_WIDTH x COLOR_HEIGHT @ TARGET_FPS.
            // This is unchanged from the prior build — if this profile exists,
            ...
            for (uint32_t j = 0; j < profiles->getCount(); j++) {
                ...
                if (p && p->getFormat() == OB_FORMAT_MJPG
                    ...
                    chosen_w = COLOR_WIDTH;
                    chosen_h = COLOR_HEIGHT;
                    break;
                }
            }

            // Pass 2 (additive fallback): first MJPG profile at TARGET_FPS.
            // Some firmware/profile combos do not expose MJPG 1280x800 — older
            // Pis logged 1280x720 here. Falling back to the next-best MJPG@30
            // profile keeps the recorder functional. The depth path is
            // unchanged: depth still requires its exact profile.
            if (!chosen) {
                for (uint32_t j = 0; j < profiles->getCount(); j++) {
                    auto p = profiles->getProfile(j)->as<ob::VideoStreamProfile>();
                    if (p && p->getFormat() == OB_FORMAT_MJPG
                        && p->getFps() == TARGET_FPS) {
                        chosen   = profiles->getProfile(j);
                        chosen_w = (int)p->getWidth();
                        chosen_h = (int)p->getHeight();
                        std::cerr << "[warn] No MJPG " << COLOR_WIDTH << "x" << COLOR_HEIGHT
                                  << "@" << TARGET_FPS
                                  << " — falling back to MJPG " << chosen_w << "x"
                                  << chosen_h << "@" << TARGET_FPS << std::endl;
                        break;
                    }
                }
            }
            ...
            if (chosen) {
                ...
                std::cout << "Enabled stream: Color MJPG " << chosen_w << "x" << chosen_h
                          << "@" << TARGET_FPS << std::endl;
            } else {
                std::cerr << "[error] No MJPG @" << TARGET_FPS
                          << " color profile available on this device." << std::endl;
                return EXIT_FAILURE;
            }
