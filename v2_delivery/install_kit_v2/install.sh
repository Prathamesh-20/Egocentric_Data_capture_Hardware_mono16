...
# Optional helper binary (additive — for FOV pre-check)
STREAM_HELPER_NAME="ob_color_jpeg_stream"
STREAM_HELPER_DIR_NAME="2.stream.color_jpeg_stdout"
STREAM_HELPER_SRC_DIR="$KIT_DIR/sdk_addon/$STREAM_HELPER_DIR_NAME"
STREAM_HELPER_DST_DIR="$SDK_ROOT/examples/src/$STREAM_HELPER_DIR_NAME"
STREAM_HELPER_INSTALL="$SDK_ROOT/bin/$STREAM_HELPER_NAME"
...
# ---------- 2b. Stream helper (additive — optional FOV pre-check binary) ---
#
# This section is purely additive. It deploys a sibling SDK example folder
# next to 2.device.record.nogui_mcap and inserts an add_subdirectory line
# for it in the parent CMakeLists. If the kit's sdk_addon dir is missing
# (e.g. a partial unpack), this whole section is skipped — the recorder
# still installs and the project's FOV check just remains disabled.

if [ -d "$STREAM_HELPER_SRC_DIR" ] \
   && [ -f "$STREAM_HELPER_SRC_DIR/$STREAM_HELPER_NAME.cpp" ] \
   && [ -f "$STREAM_HELPER_SRC_DIR/CMakeLists.txt" ]; then

    log "=== Stream helper (additive) ==="
    mkdir -p "$STREAM_HELPER_DST_DIR"

    for f in "$STREAM_HELPER_NAME.cpp" CMakeLists.txt; do
        if [ -f "$STREAM_HELPER_DST_DIR/$f" ] \
           && cmp -s "$STREAM_HELPER_SRC_DIR/$f" "$STREAM_HELPER_DST_DIR/$f"; then
            skip "$f unchanged in $STREAM_HELPER_DST_DIR"
        else
            cp "$STREAM_HELPER_SRC_DIR/$f" "$STREAM_HELPER_DST_DIR/$f"
            ok "copied $f -> $STREAM_HELPER_DST_DIR/"
        fi
    done

    if grep -qE "^[[:space:]]*add_subdirectory\($STREAM_HELPER_DIR_NAME\)[[:space:]]*$" \
            "$PARENT_CMAKE"; then
        skip "parent CMakeLists already contains add_subdirectory($STREAM_HELPER_DIR_NAME)"
    else
        if ! grep -qE '^[[:space:]]*add_subdirectory\(2\.device\.record\.nogui_mcap\)[[:space:]]*$' \
                "$PARENT_CMAKE"; then
            warn "no add_subdirectory(2.device.record.nogui_mcap) line to insert near; skipping helper"
        else
            cp "$PARENT_CMAKE" "$PARENT_CMAKE.bak.$TS.streamhelper"
            awk -v subdir="$STREAM_HELPER_DIR_NAME" '
                BEGIN { inserted = 0 }
                {
                    print
                    if (!inserted && $0 ~ /^[[:space:]]*add_subdirectory\(2\.device\.record\.nogui_mcap\)[[:space:]]*$/) {
                        indent = $0
                        sub(/add_subdirectory\(2\.device\.record\.nogui_mcap\).*$/, "", indent)
                        printf "%sadd_subdirectory(%s)\n", indent, subdir
                        inserted = 1
                    }
                }
            ' "$PARENT_CMAKE.bak.$TS.streamhelper" > "$PARENT_CMAKE"
            grep -qE "^[[:space:]]*add_subdirectory\($STREAM_HELPER_DIR_NAME\)[[:space:]]*$" \
                    "$PARENT_CMAKE" \
                || die "failed to insert add_subdirectory($STREAM_HELPER_DIR_NAME)"
            ok "inserted add_subdirectory($STREAM_HELPER_DIR_NAME)"
        fi
    fi

    BUILD_STREAM_HELPER=1
else
    skip "stream helper sources not present in kit — FOV check will be unavailable"
    BUILD_STREAM_HELPER=0
fi
...
# Build and install the optional stream helper (additive). Failures here do
# NOT fail the overall install — the recorder still works without the helper.
if [ "${BUILD_STREAM_HELPER:-0}" = "1" ]; then
    log "=== Build stream helper ==="
    if cmake --build "$BUILD_DIR" --target "$STREAM_HELPER_NAME" -j"$(nproc)"; then
        BUILT_HELPER="$(find "$BUILD_DIR" -type f -name "$STREAM_HELPER_NAME" -perm -u+x 2>/dev/null | head -n1)"
        if [ -n "$BUILT_HELPER" ]; then
            install -m 0755 "$BUILT_HELPER" "$STREAM_HELPER_INSTALL"
            ok "installed: $STREAM_HELPER_INSTALL"
        else
            warn "stream helper built but binary not found under $BUILD_DIR — skipping install"
        fi
    else
        warn "stream helper build failed — recorder still installed; FOV check will not be available"
    fi
fi
