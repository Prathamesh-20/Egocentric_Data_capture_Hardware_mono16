...
# Latest-frame buffer for FOV MJPEG stream (updated by frame_cb from SessionV2)
_latest_fov_frame:     Optional[bytes] = None
_latest_fov_frame_lock = threading.Lock()
_latest_fov_frame_ts:  float = 0.0
...
def start_fov_check():
    """
    Legacy endpoint — FOV now runs automatically before every segment.
    This just marks the UI as ready. The actual FOV feed is served on /fov-stream
    while a session is running and a pre-segment check is in progress.
    """
    ...
    if state["status"] in ("recording", "converting", "session_active", "frame_check",
                           "fov_checking", "fov_between_segments"):
        return JSONResponse({"error": "FOV already running or session active"}, 400)

    _set_state(status="fov_passed",
               message="Ready to record — FOV check runs automatically before each segment",
               fov_result="auto")
    return {"ok": True}


def _update_fov_frame(frame_bgr: np.ndarray, detected: bool):
    """Called from SessionV2 via on_frame_check. Encodes frame as JPEG for MJPEG stream."""
    global _latest_fov_frame, _latest_fov_frame_ts
    try:
        ok, jpg = cv2.imencode(".jpg", frame_bgr, [int(cv2.IMWRITE_JPEG_QUALITY), 75])
        if not ok:
            return
        with _latest_fov_frame_lock:
            _latest_fov_frame    = jpg.tobytes()
            _latest_fov_frame_ts = time.time()
    except Exception as e:
        log.warning(f"_update_fov_frame error: {e}")


def _mjpeg_generator():
    """Yields the latest FOV frame as MJPEG. Stops when no fresh frame for >3s."""
    boundary = b"--frame"
    last_served_ts = 0.0
    idle_start = None
    while True:
        with _latest_fov_frame_lock:
            frame = _latest_fov_frame
            ts    = _latest_fov_frame_ts

        if frame and ts > last_served_ts:
            last_served_ts = ts
            idle_start = None
            yield (boundary + b"\r\n"
            ...
            # No new frame — check idle timeout
            if idle_start is None:
                idle_start = time.time()
            elif time.time() - idle_start > 3.0:
                break  # close stream — client will reconnect on next FOV phase
...
def start_session():
    ...
    _session = SessionV2(
        ...
        on_frame_check   = _update_fov_frame,
