...
class GPIOController:
    ...
    def set_upload_complete(self):
        """All uploads done — 5 distinct beeps, green LED solid (idle/ready)."""
        self._green_mode  = "on"
        self._red_mode    = "off"
        self._buzzer_mode = "off"
        log.info("[GPIO] State: UPLOAD_COMPLETE — 5x beep + Green ON")
        # 5 distinct beeps: 0.2s on, 0.2s off → ~2s total
        self.beep(count=5, on_time=0.2, off_time=0.2)
    ...
    def set_fov_checking(self):
        """FOV check in progress — both LEDs off, operator watches dashboard."""
        self._green_mode = "off"
        self._red_mode = "off"
        self._buzzer_mode = "off"
        log.info("[GPIO] State: FOV_CHECKING — LEDs OFF")

    def set_fov_failed(self):
        """FOV check failed — Red LED solid + single buzzer beep. Operator adjusts hands."""
        self._green_mode = "off"
        self._red_mode = "on"
        self.beep(count=1, on_time=0.4)
        log.info("[GPIO] State: FOV_FAILED — Red ON + beep")
